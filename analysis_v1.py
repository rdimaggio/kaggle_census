...
csv_file_object = csv.reader(open('training_filev1.csv', 'rb')) #Load in the training csv file
...
train_data, cross_val = cross_validation.train_test_split(train_data, test_size=0.8, random_state=0)

for row in train_data:
  row = convert_currency(row, [123,124,125,126,127,128,160,161,162,163,164,165])
  row = replace_NA(row)
"""
for row in train_data:
  for each in range(train_data.shape[1]):
    if row[each] == '': 
      row[each] = np.median(train_data[train_data[0::,each]\
                                           != '',each].astype(np.float))
"""
remove_list = 2,4,7,9,12,41,50,53,58,59,62,86,88,150,168
string_data = train_data[0::,remove_list]
train_data = np.delete(train_data, remove_list,1)
...
train_data = np.append(train_data, string_data,1)
